module.exports = function (app, passport) {
    ...
    app.get('/email-confirmation/:emailToken', function (req, res) {
        ...
        asynq.waterfall([
            function (done) {
                Student.findOne({ 'emailConfirmationToken': token },
                    function (err, user) {
                        if (!user) {
    ...
    app.get('/password-reset/:token', function (req, res) {
        Student.findOne({ 'resetPasswordToken': req.params.token, 'resetPasswordExpires': { $gt: Date.now() } },
            function (err, user) {
                if (!user) {
                    req.flash('passwordRecoveryMessage', 'No user found with that email.');
                    return res.redirect('/password-recovery');
